...
class genetic_algorithm(object):
    ...
    def f(self,x):
        ...
        X = data[base_list+add_list].values*x
        ...
        penalty = 1/(self.penalty*sum([abs(clf.coef_[0][i]) for i in range(len(clf.coef_[0]))]))
        #penalty = self.penalty/sum([clf.coef_[0][i]**2 for i in range(len(clf.coef_[0]))])
        return clf.score(X_test,y_test) + penalty
        #return np.sin(4*x[0])
    """
    #测试函数，正常情况下应收敛于1，对应第一项权重趋向于π/4
    def f(self,x):
        return -x[0]**2+x[0]
    """
    ...
    def iteration(self):  #不断迭代
        ...
        while N<max_iter:
            ...
            std_f = np.std(func_value_list)
            max_f = max(func_value_list)
            mean_f_list.append(mean_f)
            std_f_list.append(std_f)
            max_f_list.append(max_f)
            print(mean_f,std_f,max_f)
            ...
            if N%2==0:
                f_list.append(func_value_list)
                print('=================epoch{}=================='.format(N))
            else:
                pass
        return sons,content,mean_f_list,std_f_list,max_f_list,f_list   #得到整个过程中的总体平均适应度与最后一轮迭代的权重
    
    def func_value_plot(self):
        sons,content,mean_f_list,std_f_list,max_f_list = self.iteration()
        fig = plt.figure(figsize=(15,5))
        plt.subplot(1,3,1)
        plt.plot(mean_f_list)
        plt.xlabel('iteration')
        plt.ylabel('scores')
        plt.title('mean of utility function')
        
        plt.subplot(1,3,2)
        plt.plot(std_f_list)
        plt.xlabel('iteration')
        plt.ylabel('std')
        plt.title('std of utility function')
        
        plt.subplot(1,3,3)
        plt.plot(max_f_list)
        plt.xlabel('iteration')
        plt.ylabel('max_f')
        plt.title('maximum of utility function')
        
if __name__ == '__main__':
    #数据处理
    data = pd.read_excel('回归数据2.xlsx')
    base_list = ['实际资本','参考利率','经营时间']
    add_list = ['银行存管1','加入协会','ICP认证','异常经营次数']
    base = data[base_list]
    add = data[add_list]
    y = data['label']
    #迭代计算，population为100项，
    ga = genetic_algorithm(100,len(add_list+base_list),2)
    result,content,mean_f_list,std_f_list,max_f_list,f_list = ga.iteration()
    #ga.func_value_plot()